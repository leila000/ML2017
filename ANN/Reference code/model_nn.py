...
class cifarMLPParallel(chainer.Chain):

    """An example of model-parallel MLP.
    This chain combines four small MLPs on two different devices.
    """
    def __init__(self, n_in, n_units, n_out):
        super(cifarMLPParallel, self).__init__(
            first0=cifarMLP(n_in, n_units // 2, n_units).to_gpu(0),
            first1=cifarMLP(n_in, n_units // 2, n_units).to_gpu(1),
            second0=cifarMLP(n_units, n_units // 2, n_out).to_gpu(0),
            second1=cifarMLP(n_units, n_units // 2, n_out).to_gpu(1),
        )
